...
void Reportes::reporteLote(int lote_id) {
    ...
    std::cout << "  Consumo: " << resultado.first << " sacks (Bs. " << std::fixed << std::setprecision(2) << resultado.second << ")" << std::endl;
}
...
void Reportes::reporteFinanciero() {
    ...
    std::cout << "Precio del USD: Bs. " << std::fixed << std::setprecision(2) << config.precio_dolar << std::endl;
    std::cout << "Precio por Kg (VES): Bs. " << config.precio_kg_ves << std::endl;
    double precio_usd = calcularPrecioUSD(config.precio_kg_ves, config.precio_dolar);
    std::cout << "Precio por Kg (USD): $" << std::fixed << std::setprecision(2) << precio_usd << std::endl << std::endl;
    ...
    for (auto& inv : Inversores::listar()) {
        std::cout << "  " << inv.nombre << ": Bs. " << std::fixed << std::setprecision(2) << inv.cantidad_invertida << std::endl;
    }
    double total_inv = Inversores::totalInvertido();
    std::cout << "Total invertido: Bs. " << std::fixed << std::setprecision(2) << total_inv << std::endl;
}
...
void Reportes::reporteInventario() {
    ...
    for (auto& h : Herramientas::listar()) {
        std::cout << "  " << h.nombre << ": " << h.cantidad << " x Bs. " << std::fixed << std::setprecision(2) << h.precio_unitario << " = Bs. " << h.cantidad * h.precio_unitario << std::endl;
    }
    std::cout << "Total herramientas: Bs. " << std::fixed << std::setprecision(2) << Herramientas::totalInvertido() << std::endl;
}
...
void Reportes::reporteConsumo(int lote_id) {
    ...
    auto resultado = Alimentacion::totalConsumo(lote_id);
    std::cout << "Total: " << resultado.first << " sacks - Costo: Bs. " << std::fixed << std::setprecision(2) << resultado.second << std::endl;
}
